...
update role set description=name;
...
create table admin(
	...
	list_order tinyint unsigned default 0,
	status tinyint default 1,
	/* -1 删除 0 失效 1 正常 */
	create_time timestamp default current_timestamp,
	update_time timestamp on update current_timestamp,
...
insert into admin(username,realname,password,role)
	values('admin','administrator',md5('123456'),1);
insert into admin(username,realname,password,role)
	values('mqs','ma quansheng',md5('123456'),1);
...
create table auth(
	...
	granted tinyint unsigned default 1,
	create_time timestamp default current_timestamp,
	update_time timestamp on update current_timestamp,
	delete_time int unsigned default null,
...
create table user(
	...
	xiaoqu_id tinyint unsigned default 0,
	addr varchar(64),
...
create table notifies(
	...
	/* 发布人 */
	is_position tinyint unsigned default 0,
	/* 是否推荐到首页轮播 */
...
insert into notifies(title,type_id,xiaoqu_id,category_id,author,is_position,is_hot)
	values('',floor(2+rand()*4),floor(1+rand()*2),floor(1+rand()*20),floor(1+rand()*5),round(rand()),round(rand()));

update notifies set title=concat('社区资讯 ',id) where type_id=2;
update notifies set title=concat('居委会资讯 ',id) where type_id=3;
update notifies set title=concat('业委会资讯 ',id) where type_id=4;
update notifies set title=concat('物业资讯 ',id) where type_id=5;
update notifies set content=concat(title,' 内容'),description=concat(title,' 摘要');
update notifies set category_id=floor(5+rand()*4) where type_id=2;
update notifies set category_id=floor(9+rand()*5) where type_id=3;
update notifies set category_id=floor(14+rand()*4) where type_id=4;
update notifies set category_id=floor(18+rand()*8) where type_id=5;


select n.id,n.title,n.description,n.create_time,a.username
from notifies n
...
create table feereport(
	...
	/* 缴费金额 */
	is_owner_pay tinyint unsigned default 1,
	/* 是否用户本人缴纳。0 否 1 是 */
...
create table backreport(
	...
	amount numeric(8,2) default 0,
	/* 退费金额 */
	is_back2owner tinyint unsigned default 1,
	/* 是否退还给用户本人。0 否 1 是 */
	back2 varchar(32),
	/* 非退还给用户本人，退还人员姓名 */
	oprator int unsigned,
...
drop table if exists proper_type;
create table proper_type(
	id int unsigned auto_increment,
	name varchar(32) not null unique,
	summary varchar(100),
	status tinyint default 1,
...
create table properties(
	...
	addr varchar(32) unique,
	/* 房产楼门号；车辆牌照号 */
	property_type tinyint unsigned default 1,
	/* 物业类型 1 房产 2 车位 */
	summary varchar(200),
	/* 描述 */
	status tinyint default 1,
	fee_status tinyint default 1,
	/* 缴费状态。0 停缴 1 正常 2 欠费 3 优惠 */
	discount numeric(8,2) default 0,
	/* 优惠金额 */
	is_lease tinyint default 0,
	/* 是否已出租 */
	is_arrears tinyint default 0,
	/* 是否欠费 */
	begin_date date,
	/* 欠费开始日期 */
	amount numeric(8,2),
...
insert into properties(user_id,addr,property_type)
	values(1,'新凯东里6-3-401',1),(2,'津N67185',2);

	
/* 查询用户名下的物业 */

select id,addr,(case property_type when 1 then '房产' when 2 then '车位' else '未知' end) as property_name,is_lease,is_arrears,status from properties;
...
from feereport r,properties p,feetype t
where (r.property_id=p.id and r.type_id=t.id) and r.property_id in (select id from properties where user_id=1);


select r.id,p.addr,pay_time,t.name,r.amount,(case when r.is_owner_pay=1 then u.username else r.payer_name end) payer from feereport r,properties p,feetype t,user u where (r.property_id=p.id and r.type_id=t.id) and u.id=1 and r.id = 3;
